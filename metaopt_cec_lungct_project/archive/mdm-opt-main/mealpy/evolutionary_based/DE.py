...
class BinaryDE(OriginalDE):
    """
    The binary version of: Differential Evolution (DE)
    """

    def __init__(self, epoch: int = 10000, pop_size: int = 100, wf: float = 0.1, cr: float = 0.9, strategy: int = 0, transfer_func=None, **kwargs: object) -> None:
        if transfer_func is None:
            raise ValueError("transfer_func must be provided for BinaryDE.")
        super().__init__(epoch, pop_size, wf, cr, strategy, **kwargs)
        self.transfer_func = transfer_func

    def mutation__(self, current_pos, new_pos):
        condition = self.generator.random(self.problem.n_dims) < self.cr
        pos_new = np.where(condition, new_pos, current_pos)
        return self.transfer_func(pos_new)
