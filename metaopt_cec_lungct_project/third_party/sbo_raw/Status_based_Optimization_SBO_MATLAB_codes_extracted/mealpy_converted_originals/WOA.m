function [best_score, best_pos, curve] = WOA(pop_size, max_iter, lb, ub, dim, fobj, opts) %#ok<INUSD>
% WOA MATLAB port of mealpy.swarm_based.WOA.OriginalWOA.
    ...
    if pop_size < 5
        error('WOA requires pop_size >= 5.');
    end
    if max_iter < 1
        error('WOA requires max_iter >= 1.');
    end
    ...
    for epoch = 1:max_iter
        ...
        for idx = 1:pop_size
            ...
            if rand < 0.5
                if abs(A) < 1
                    D = abs(C .* g_best - X(idx, :));
                    pos_new = g_best - A .* D;
                else
                    x_rand = lb + rand(1, dim) .* (ub - lb);
                    D = abs(C .* x_rand - X(idx, :));
                    pos_new = x_rand - A .* D;
                end
            else
                D1 = abs(g_best - X(idx, :));
                pos_new = g_best + exp(l) * cos(2 * pi * l) .* D1;
            end
