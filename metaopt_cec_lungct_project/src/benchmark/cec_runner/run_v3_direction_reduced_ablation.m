...
function phase_cfg = build_phase_cfg(cfg, phase)
    phase_cfg = struct();
    phase_cfg.suites = {cfg.suite};
    phase_cfg.algorithms = cfg.algorithms;
    phase_cfg.func_ids = struct(cfg.suite, cfg.reduced_func_ids);
    phase_cfg.dim = cfg.dim;
    phase_cfg.pop_size = cfg.pop_size;
    phase_cfg.maxFEs = cfg.maxFEs;
    phase_cfg.rng_seed = cfg.rng_seed;
    phase_cfg.save_curve = cfg.save_curve;
    phase_cfg.save_mat = cfg.save_mat;
    phase_cfg.save_csv = cfg.save_csv;
    phase_cfg.plot = cfg.plot;

    if strcmpi(phase, 'smoke')
        phase_cfg.runs = cfg.smoke_runs;
        phase_cfg.result_root = fullfile('results', 'benchmark', 'v3_direction_reduced_smoke');
        phase_cfg.experiment_name = sprintf('v3_direction_reduced_smoke_%s', cfg.timestamp);
    else
        phase_cfg.runs = cfg.formal_runs;
        phase_cfg.result_root = fullfile('results', 'benchmark', 'v3_direction_reduced_formal');
        phase_cfg.experiment_name = sprintf('v3_direction_reduced_formal_%s', cfg.timestamp);
    end
end
