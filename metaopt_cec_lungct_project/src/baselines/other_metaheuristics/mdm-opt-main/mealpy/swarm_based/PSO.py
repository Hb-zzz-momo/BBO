...
class BinaryPSO(OriginalPSO):
    """
    The binary version of: Particle Swarm Optimization (PSO)
    """

    def __init__(self, epoch: int = 10000, pop_size: int = 100, c1: float = 2.05, c2: float = 2.05, w: float = 0.4, transfer_func=None, **kwargs: object) -> None:
        if transfer_func is None:
            raise ValueError("transfer_func must be provided for BinaryPSO.")
        super().__init__(epoch, pop_size, c1, c2, w, **kwargs)
