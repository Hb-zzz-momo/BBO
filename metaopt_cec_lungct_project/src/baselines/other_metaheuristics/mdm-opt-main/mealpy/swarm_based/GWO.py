...
class BinaryGWO(OriginalGWO):
    """
    The binary version of: Grey Wolf Optimizer (GWO)
    Args:
        epoch (int): Maximum number of iterations. Default: 10000.
        pop_size (int): Population size. Default: 100.
        transfer_func (callable): Transfer function to convert continuous to binary. (Required)
        **kwargs: Additional arguments for the parent class (OriginalGWO).
    """
    def __init__(self, epoch: int = 10000, pop_size: int = 100, transfer_func=None, **kwargs: object) -> None:
        # Validate that transfer_func is provided (if no default)
        if transfer_func is None:
            raise ValueError("transfer_func must be provided for BinaryGWO.")
        super().__init__(epoch, pop_size, **kwargs)
    ...
    def evolve(self, epoch):
        ...
        for idx in range(0, self.pop_size):
            ...
            pos_new = (X1 + X2 + X3) / 3.0
            # pos_new = self.correct_solution(pos_new)
            pos_new = self.transfer_func(pos_new)
            agent = self.generate_empty_agent(pos_new)
            pop_new.append(agent)
            if self.mode not in self.AVAILABLE_MODES:
                agent.target = self.get_target(pos_new)
