...
class BinarySSA(OriginalSSA):
    """
    The binary version of: Sparrow Search Algorithm (SSA)
    """
    def __init__(self, epoch: int = 10000, pop_size: int = 100, transfer_func=None, **kwargs: object) -> None:
        if transfer_func is None:
            raise ValueError("transfer_func must be provided for BinarySSA.")
        ...
        for idx in range(0, self.pop_size):
            ...
            if idx < self.n1:
                ...
                if idx > int(self.pop_size / 2):
                    ...
                    x_new = g_best.solution + np.matmul(np.abs(self.pop[idx].solution - g_best.solution), A1)
            # pos_new = self.correct_solution(x_new)
            pos_new = self.transfer_func(x_new)
            agent = self.generate_empty_agent(pos_new)
            pop_new.append(agent)
            if self.mode not in self.AVAILABLE_MODES:
                agent.target = self.get_target(pos_new)
