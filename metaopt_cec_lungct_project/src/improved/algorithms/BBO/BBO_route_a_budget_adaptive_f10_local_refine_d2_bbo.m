function [best_fitness, best_solution, Convergence_curve] = BBO_route_a_budget_adaptive_f10_local_refine_d2_bbo(N, Max_iteration, lb, ub, dim, fobj)
% BBO_route_a_budget_adaptive_f10_local_refine_d2_bbo
% D2 version: only tighten the local-refine mechanism group for small-scale elite-zone polishing.
    ...
    % D2: smaller-scale elite-zone refine only.
    refine_cfg = struct();
    refine_cfg.enable_progress = 0.72;
    refine_cfg.elite_ratio = 0.10;
    refine_cfg.apply_prob = 0.15;
    refine_cfg.pull_best = 0.12;
    refine_cfg.sigma_scale = 0.006;
    refine_cfg.good_zone_gap_ratio = 0.18;
    ...
    for t = 1:Max_iteration
        ...
        if progress >= refine_cfg.enable_progress
            ...
            elite_vals = fitness(targets);
            gap_ref = max(1e-12, max(elite_vals) - min(elite_vals));
            step_sigma = refine_cfg.sigma_scale * (1 - progress) * (ub - lb);

            for k = 1:numel(targets)
                i = targets(k);
                gap_ratio = (fitness(i) - best_fitness) / max(1e-12, gap_ref);
                if gap_ratio > refine_cfg.good_zone_gap_ratio
                    continue;
                end
