function [best_fitness, best_solution, Convergence_curve] = BBO_route_a_late_pbest_bbo(N, Max_iteration, lb, ub, dim, fobj)
% BBO_route_a_late_pbest_bbo
% Direction 1: CURRENT_BBO + late PBEST refinement.
% Design constraints:
% 1) Early/mid stage keeps CURRENT behavior.
% 2) PBEST is activated only in late stage and only on elite individuals.
% 3) PBEST candidate uses greedy acceptance only.
    ...
    for t = 1:Max_iteration
        ...
        diversity_ratio = mean(std(population, 0, 1)) / (mean(ub - lb) + eps);
        late_trigger = (progress >= late_progress_start) && ...
            (stagnant_gens >= stagnation_patience || diversity_ratio <= diversity_threshold);
        ...
        for i = 1:N
            ...
            if late_trigger && ismember(i, elite_idx)
                pbest_ref = population(pbest_pool(randi(numel(pbest_pool))), :);
                pbest_trial = xi + 0.25 * (pbest_ref - xi) + 0.20 * F * (population(r1, :) - population(r2, :));
